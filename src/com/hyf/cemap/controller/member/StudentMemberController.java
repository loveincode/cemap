...
@Controller
@RequestMapping(value = "/studentmember")
public class StudentMemberController {
    ...
    public String data(HttpServletRequest request, HttpServletResponse response) {
        ...
        String sadministrativeClassId = request.getParameter("administrativeClassId");
        ...
        if (userDetailServiceImpl.findUserAuthorities(memberonline).toString().equals("[ROLE_TEACHER]")) {
            try {
                ...
                // 总记录数
                Integer recordsTotal = memberService.listMemberByinfo2(null,null,classIds,-1,4,-1,-1).size();
                // 关键字过滤后总记录数
                Integer recordsFiltered = memberService.listMemberByinfo2(username,realname,classIds,administrativeClassId,4,-1, -1).size();
                dataTableUtil.setResult(recordsTotal, recordsFiltered,
                        memberService.listMemberByinfo2(username,realname,classIds,administrativeClassId,4,dataTableUtil.getPage(),
            ...
            try {
                // 总记录数
                Integer recordsTotal = memberService.listMemberByinfo(null,null,4,-1,-1).size();
                // 关键字过滤后总记录数
                Integer recordsFiltered = memberService.listMemberByinfo(username,realname,4,-1, -1).size();
                dataTableUtil.setResult(recordsTotal, recordsFiltered,
                        memberService.listMemberByinfo(username,realname,4,dataTableUtil.getPage(),
                                    dataTableUtil.getLength()));
            } catch (Exception e) {
                e.printStackTrace();
            }
    ...
    public ModelAndView modifyPage(
        ...
        try {
            ...
            modelAndView.setViewName("member/student/modify");
        } catch (Exception e) {
            e.printStackTrace();
        }
        return modelAndView;
    ...
    public @ResponseBody String delete(
        ...
        try {
            ...
            //删除用户将删除用户所有信息 毕业生信息 档案信息 就业信息
            GraduateInformation ginfo =  graduateInformationService.getByStudentId(member.getUsername());
            ginfo.setDeleted(true);
            graduateInformationService.update(ginfo);
            GraduationFile gfile = graduationFileService.getByStudentId(member.getUsername());
            gfile.setDeleted(true);
            graduationFileService.update(gfile);
            EmploymentInformation einfo = employmentInformationService.getByStudentId(member.getUsername());
            einfo.setDeleted(true);
            employmentInformationService.update(einfo);
    ...
    public ModelAndView passwordModifyPage(
        ...
        ModelAndView modelAndView = new ModelAndView();
            try {
                Member member = memberService.getByUuid(uuid);
                modelAndView.addObject("member", member);
                modelAndView.setViewName("member/student/passwordmodify");
